...
--Создадим дополнительную связь между исполнителями и треками
CREATE TABLE IF NOT EXISTS Trecks_Singers (
	treck_id INTEGER REFERENCES Trecks(treck_id),
	singer_id INTEGER REFERENCES Singers(singer_id),
	CONSTRAINT ts PRIMARY KEY (treck_id, singer_id)
);

--Создадим дополнительную связь между альбомами и треками
CREATE TABLE IF NOT EXISTS Albums_Trecks (
	album_id INTEGER references Albums_Trecks(album_id),
	treck_id INTEGER REFERENCES Trecks(treck_id),
	CONSTRAINT ts PRIMARY KEY (album_id, treck_id)
);


-- Предыдущие CREATE-запросы
